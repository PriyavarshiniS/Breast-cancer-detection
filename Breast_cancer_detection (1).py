...
pd.value_counts(shape1)
import pandas as pd
class_counts = mass_test['mass_shape'].value_counts()
least = class_counts[class_counts<40].index.tolist()
mass_test['mass_shape']=mass_test['mass_shape'].apply(lambda x:'others' if x in least else x)
print(mass_test['mass_shape'].value_counts())
...
# Model Building
num_classes = mass_test_combined['mass_shape'].nunique()
...
predictions = Dense(len(mass_train['mass_shape'].unique()), activation='sigmoid')(x)  # Use sigmoid for multi-label classification
...
class_names = mass_train['mass_shape'].unique().tolist()
